...
def get_parser():
    parser = argparse.ArgumentParser(description='Download ABCD participant data '
             'and run MRIQC and fMRIPREP docker images')
...
def main(argv=None):
    ...
    rs_files = sorted(glob(op.join(args.bids_dir, 'derivatives', 'fmriprep_post-process', args.sub, args.ses, 'rest', '*-clean.nii.gz')))
    for rs_file in rs_files:
        out_dir = op.join(args.bids_dir, 'derivatives', 'fmriprep_post-process_{}'.format(roi_out_dirname), args.sub, args.ses, op.basename(rs_file).split('.')[0])
        os.makedirs(out_dir, exist_ok=True)

        smooth_file = '{}-smooth+clean.nii.gz'.format(rs_file.rstrip('-clean.nii.gz'))
        mask_fname = '{}-brain_mask.nii.gz'.format(rs_file.split('-preproc_bold-clean.nii.gz')[0])
        ...
        for i, roi in enumerate(rois):
            ...
            cmd = '3dresample -prefix {out_fname} \
                              -master {rs_file} \
                              -inset {roi}'.format(out_fname=op.join(out_dir, '{}.nii'.format(roi_name)),
                                                   rs_file=rs_file,
                                                   roi=roi)
            os.system(cmd)
            ...
            cmd='3dmaskave -q \
                           -mask {mask} \
                           {rs_file} > {rs_timeseries}'.format(mask=op.join(out_dir, '{}.nii'.format(roi_name)),
                                                               rs_file=rs_file,
                                                               rs_timeseries=op.join(out_dir, '{}.txt'.format(roi_name)))
            os.system(cmd)

            stim_info += '-stim_file {num} {rs_timeseries} '.format(num=i+1,
                                                                      rs_timeseries=op.join(out_dir, '{}.txt'.format(roi_name)))
            stim_info += '-stim_label {num} "{roi_name}" '.format(num=i+1,
                                                                    roi_name=roi_name)
        ...
        cmd='3dDeconvolve -input {rs_smooth_fn} \
            	    	  ...
            	    	  -basis_normall 1 \
            	    	  {stim_info} \
            	    	  -x1D {matrix_fname}'.format(rs_smooth_fn=smooth_file,
        ...
        cmd='3dREMLfit -matrix {matrix_fname} \
                       -input {rs_smooth_fn} \
      	               ...
      	               -tout \
      	               -Rbuck {out_bucket} \
                       -verb'.format(matrix_fname=matrix_fname,
        ...
        cmd='3dcalc -a {out_bucket}+tlrc \
                    -expr "atanh(a)" \
                    -prefix {out_bucket_z}'.format(out_bucket=out_bucket,
                                                   out_bucket_z=out_bucket_z)
        os.system(cmd)
