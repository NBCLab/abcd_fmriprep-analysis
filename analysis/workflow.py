...
def get_parser():
    ...
    parser.add_argument(
        ...
        required=False,
        dest="task",
        help="Task ID (mid, nback, sst, rest)",
    ...
    parser.add_argument(
        ...
        dest="roi",
        nargs="+",
        help="Full path to ROI for resting-state analysis",
        default=None,
    )
    return parser
...
def main(argv=None):
    ...
    if args.task != "rest":
        cmd = "python3 {code_dir}/task/post_process_task.py -b {bids_dir} \
                                                            ...
                                                            --ses {ses} \
                                                            --task {task}".format(
            code_dir=code_dir, bids_dir=args.bids_dir, sub=args.sub, ses=args.ses, task=args.task
        )
        os.system(cmd)
        ...
        cmd = "python3 {code_dir}/rest/3dTproject_denoise.py -b {bids_dir} \
                                                           --sub {sub} \
                                                           --ses {ses}".format(
            code_dir=code_dir, bids_dir=args.bids_dir, sub=args.sub, ses=args.ses
        )
        os.system(cmd)
        if args.roi:
            ...
            cmd = "python3 {code_dir}/rest/rsfc-old.py --bids_dir {bids_dir} \
                                                 ...
                                                 --ses {ses} \
                                                 --roi {roi}".format(
                code_dir=code_dir, bids_dir=args.bids_dir, sub=args.sub, ses=args.ses, roi=rois
            )
            os.system(cmd)
