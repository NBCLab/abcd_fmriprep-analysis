...
def main(argv=None):
    ...
    output_dir = op.join(
        args.bids_dir, "derivatives/fmriprep_post-process", args.sub, args.ses, "rest"
    )
    ...
    for scan in scans:
        ...
        nii_fn = op.join(
            ...
            args.ses,
            "func",
            "{}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz".format(niifile),
        )
        mask_fn = op.join(
            fmriprep_dir,
            "fmriprep",
            ...
            args.ses,
            "func",
            "{}_space-MNI152NLin2009cAsym_desc-brain_mask.nii.gz".format(niifile),
        )
        ...
        if not op.isfile(op.join(output_dir, op.basename(mask_fn))):
            ...
            js_fname = op.join(
                args.bids_dir, args.sub, args.ses, "func", "{}_bold.json".format(niifile)
            )
            ...
            acompcor_regressors = op.join(
                output_dir, "{0}_acompcor.1D".format(acompcor_regressors)
            )
            ...
            censor_out_name = op.join(output_dir, "{}_motion_censoring.1D".format(niifile))
            np.savetxt(censor_out_name, censor_data, fmt="%.1d")
            tr_censor = pd.read_csv(
                op.join(output_dir, "{0}_motion_censoring.1D".format(niifile)), header=None
            )
            tr_keep = tr_censor.index[tr_censor[0] == 1].tolist()

            motion_regressors = motion_parameters(scan, derivatives=True).drop(
                index=np.arange(trs_to_delete)
            )
            motion_out_name = op.join(output_dir, "{}_motion_parameters.1D".format(niifile))
            ...
            os.system(cmd)
            cmd = "3dTcat -prefix {out_file} {in_file}'{tr_keep}'".format(
                in_file=op.join(output_dir, "tmp.nii"),
                tr_keep=tr_keep,
