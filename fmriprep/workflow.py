...
def main(argv=None):
    ...
    fmriprep_image = op.join(singularity_images_home, 'poldracklab-fmriprep_{fmriprep_version}.sif'.format(fmriprep_version=fmriprep_version))
    fs_license_file = op.join(op.dirname(singularity_images_home), 'cis-processing', 'fs_license.txt')
    ...
    if not op.isdir(op.join(dicom2bids_output_dir, args.sub)):
        cmd='singularity run --cleanenv \
            ...
            rm -rf {dicom2bids_work_dir}'.format(sub=args.sub,
                                                 ...
                                                 output_file='{}.txt'.format(op.join(op.dirname(args.bids_dir), 'code', 'dicom2bids', args.sub)))
    ...
    if not op.isdir(op.join(fmriprep_output_dir, 'fmriprep', args.sub)):
