...
def main(argv=None):
    ...
    for run in ['1', '2', '1+2']:
        in_file = op.join(args.bids_dir,
                        ...
                        args.ses,
                        'MID',
                        '{sub}_{ses}_task-MID_run-{run}_space-MNI152NLin2009cAsym_res-2_desc-smooth.nii.gz'.format(sub=args.sub, ses=args.ses, run=run))
        ...
        mask_fn = op.join(args.bids_dir,
                        ...
                        args.ses,
                        'MID',
                        '{sub}_{ses}_task-MID_run-{run}_space-MNI152NLin2009cAsym_res-2_desc-mask.nii.gz'.format(sub=args.sub, ses=args.ses, run=run))
        ...
        bucket_fn = op.join(args.bids_dir,
                        ...
                        args.ses,
                        'MID',
                        '{sub}_{ses}_task-MID_run-{run}_space-MNI152NLin2009cAsym_res-2_desc-REMLbucket+SPMG'.format(sub=args.sub, ses=args.ses, run=run))
